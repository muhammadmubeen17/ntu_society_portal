$(document).ready(function () {
    // Function to show or hide the Registration Number input field
    function toggleRegistrationNumber() {
        const selectedUserRole = $('input[name="user_role"]:checked').val();
        console.log(selectedUserRole)
        if (selectedUserRole === 'member' || selectedUserRole === 'president') {
            $('#registrationNumberWrapper').show();
        } else {
            $('#registrationNumberWrapper').hide();
        }
    }

    // Add event listener to radio buttons for user_role using jQuery
    $('input[name="user_role"]').on('change', toggleRegistrationNumber);

    // Initially, call the function to set the initial state based on the selected user role
